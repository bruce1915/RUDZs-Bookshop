...
def view_cart():
    ...
    cart_items = CartItem.query.filter_by(user_id=user.sno).all()
    cart_books = []
    total_price = 0
    ...
    session['cart_items'] = cart_books
    session['cart_total'] = total_price
    
    return render_template('cart.html', cart_items=cart_books, total_price=total_price)
...
def add_to_cart(book_id):
    ...
    if book.stock < quantity:
        flash(f'Sorry, only {book.stock} copies available', 'error')
        return redirect(f'/featured')
    
    existing_item = CartItem.query.filter_by(user_id=user.sno, book_id=book_id).first()
    ...
    if existing_item:
        ...
        cart_item = CartItem(user_id=user.sno, book_id=book_id, quantity=quantity)
...
def update_cart(item_id):
    ...
    if not user or cart_item.user_id != user.sno:
...
def checkout():
    ...
    return render_template('checkout.html', cart_items=cart_books, total_price=total_amount)
...
def process_payment():
    ...
    if payment_method == 'credit_card' and (not card_number or not card_holder or not expiry_date or not cvv):
    ...
    for item in cart_items:
        book = Book.query.get(item.book_id)
        if book:
            total_amount += float(book.price) * item.quantity
    ...
    for item in cart_items:
        ...
        if book:
            ...
            book.total_sells += item.quantity
    ...
    return redirect('/order_confirmation/' + str(order.id))
...
def order_confirmation(order_id):
    ...
    return render_template('order_confirmation.html', order=order, order_items=order_items, payment=payment)
...
def order_history():
    ...
    orders = Order.query.filter_by(user_id=user.sno).order_by(Order.order_date.desc()).all()
    ...
    return render_template('order_history.html', orders=orders_with_items)
